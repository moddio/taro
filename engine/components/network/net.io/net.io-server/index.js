 ...
 * @type {*}
 */
NetIo.Class = typeof (IgeClass) !== 'undefined'
	? IgeClass
	: (function () {
		...
		var log = function (text, type, obj) {
			...
			if (type === 'error') {
				...
				if (NetIo._debug.throwErrors) {
					throw new Error(`${indent}Net.io *${type}* [${this._classId || this.prototype._classId}] : ${text}`);
				} else {
...
NetIo.EventingClass = typeof (IgeEventingClass) !== 'undefined'
	? IgeEventingClass
	: NetIo.Class.extend({
...
NetIo.Socket = NetIo.EventingClass.extend({
	...
	send: function (data) {
		try {
			this._socket.send(this._encode(data), function ack (err) {
     ...
     */
	_send: function (data) {
		this._socket.send(data, function ack (err) {
			// If error is not defined, the send has been completed, otherwise the error
			// object will indicate what failed.
