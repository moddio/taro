...
/*
 * Copyright (c) 2006-2009 Erin Catto http://www.physics.org
 *
...
b2Mat22.prototype =
{
	...
	GetInverse: function () {
		var a = this.ex.x; var b = this.ey.x; var c = this.ex.y; var d = this.ey.y;
		var B = new b2Mat22();
		var det = a * d - b * c;
		if (det != 0.0) {
			det = 1.0 / det;
	...
	Solve: function (b) {
		var a11 = this.ex.x; var a12 = this.ey.x; var a21 = this.ex.y; var a22 = this.ey.y;
...
b2Mat33.prototype =
{
	...
	GetSymInverse33: function (/* b2Mat33* */ M) {
		...
		var a11 = this.ex.x; var a12 = this.ey.x; var a13 = this.ez.x;
		var a22 = this.ey.y; var a23 = this.ez.y;
		var a33 = this.ez.z;
...
/*
 * Copyright (c) 2006-2010 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
...
b2ChainShape.prototype =
{
	...
	GetChildEdge: function (edge, index) {
		b2Assert(index >= 0 && index < this.m_count - 1);
...
b2PolygonShape.prototype =
{
	...
	RayCast: function (output, input,
		...
		var lower = 0.0; var upper = input.maxFraction;
		...
		b2Assert(lower >= 0.0 && lower <= input.maxFraction);
	...
	ComputeMass: function (massData, density) {
		...
		for (var i = 0; i < this.m_count; ++i) {
			...
			var ex1 = e1.x; var ey1 = e1.y;
			var ex2 = e2.x; var ey2 = e2.y;
			...
			case b2Shape.e_chain:
				{
					...
					b2Assert(index >= 0 && index < chain.m_count);
...
function b2DistanceFunc (output,
	...
	var saveA = [0, 0, 0]; var saveB = [0, 0, 0];
	var saveCount = 0;
...
function b2CollideCircles (manifold,
	...
	var rA = circleA.m_radius; var rB = circleB.m_radius;
	var radius = rA + rB;
...
function b2FindIncidentEdge (c,
	...
	b2Assert(edge1 >= 0 && edge1 < poly1.m_count);
...
function b2CollideEdgeAndCircle (manifold,
	...
	var A = edgeA.m_vertex1; var B = edgeA.m_vertex2;
...
b2EPCollider.prototype =
{
	...
	Collide: function (manifold, edgeA, xfA,
		...
		var offset0 = 0.0; var offset2 = 0.0;
		var convex1 = false; var convex2 = false;
...
/*
 * Copyright (c) 2009 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
...
b2DynamicTree.prototype =
{
	...
	DestroyProxy: function (proxyId) {
		b2Assert(proxyId >= 0 && proxyId < this.m_nodeCapacity);
	...
	Validate: function () {
		...
		while (freeIndex != b2_nullNode) {
			b2Assert(freeIndex >= 0 && freeIndex < this.m_nodeCapacity);
	...
	FreeNode: function (nodeId) {
		b2Assert(nodeId >= 0 && nodeId < this.m_nodeCapacity);
		b2Assert(this.m_nodeCount > 0);
	...
	Balance: function (iA) {
		...
		b2Assert(iB >= 0 && iB < this.m_nodeCapacity);
		b2Assert(iC >= 0 && iC < this.m_nodeCapacity);
		...
		if (balance > 1) {
			...
			b2Assert(iF >= 0 && iF < this.m_nodeCapacity);
			b2Assert(iG >= 0 && iG < this.m_nodeCapacity);
		...
		if (balance < -1) {
			...
			b2Assert(iD >= 0 && iD < this.m_nodeCapacity);
			b2Assert(iE >= 0 && iE < this.m_nodeCapacity);
	...
	ValidateStructure: function (index) {
		...
		b2Assert(child1 >= 0 && child1 < this.m_nodeCapacity);
		b2Assert(child2 >= 0 && child2 < this.m_nodeCapacity);
...
b2Fixture.prototype =
{
	...
	GetAABB: function (childIndex) {
		b2Assert(childIndex >= 0 && childIndex < this.m_proxyCount);
	...
	Synchronize: function (broadPhase, transform1, transform2) {
		...
		for (var i = 0; i < this.m_proxyCount; ++i) {
			...
			var aabb1 = new b2AABB(); var aabb2 = new b2AABB();
...
b2Contact.AddType = function (fcn,
	type1, type2) {
	b2Assert(type1 >= 0 && type1 < b2Shape.e_typeCount);
	b2Assert(type2 >= 0 && type2 < b2Shape.e_typeCount);
...
b2Contact.Destroy = function (contact) {
	...
	b2Assert(typeA >= 0 && typeB < b2Shape.e_typeCount);
	b2Assert(typeA >= 0 && typeB < b2Shape.e_typeCount);
...
/*
 * Copyright (c) 2006-2007 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
...
/*
 * Copyright (c) 2006-2011 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
...
b2RevoluteJoint.prototype =
{
	...
	InitVelocityConstraints: function (data) {
		...
		var qA = new b2Rot(aA); var qB = new b2Rot(aB);
		...
		var mA = this.m_invMassA; var mB = this.m_invMassB;
		var iA = this.m_invIA; var iB = this.m_invIB;
...
b2GearJoint.prototype =
{
	...
	InitVelocityConstraints: function (data) {
		...
		var qA = new b2Rot(aA); var qB = new b2Rot(aB); var qC = new b2Rot(aC); var qD = new b2Rot(aD);
	...
	SolvePositionConstraints: function (data) {
		...
		var JvAC = new b2Vec2(); var JvBD = new b2Vec2();
		var JwA, JwB, JwC, JwD;
		var mass = 0.0;
...
/*
 * Copyright (c) 2006-2012 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
...
b2MotorJoint.prototype =
{
	...
	SetCorrectionFactor: function (factor) {
		b2Assert(b2IsValid(factor) && factor >= 0.0 && factor <= 1.0);
...
/*
 * Copyright (c) 2011 Erin Catto http://www.physics.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
