(function (global, factory) {
    ...
    */
	...
	class b2Vec3 {
		...
		static CrossV3V3 (a, b, out) {
			const a_x = a.x; const a_y = a.y; const a_z = a.z;
			const b_x = b.x; const b_y = b.y; const b_z = b.z;
	...
	class b2Mat22 {
		...
		Solve (b_x, b_y, out) {
			const a11 = this.ex.x; const a12 = this.ey.x;
			const a21 = this.ex.y; const a22 = this.ey.y;
		...
		static AddMM (A, B, out) {
			const A_ex = A.ex; const A_ey = A.ey;
			const B_ex = B.ex; const B_ey = B.ey;
		...
		static MulMM (A, B, out) {
			const A_ex_x = A.ex.x; const A_ex_y = A.ex.y;
			const A_ey_x = A.ey.x; const A_ey_y = A.ey.y;
			const B_ex_x = B.ex.x; const B_ex_y = B.ex.y;
			const B_ey_x = B.ey.x; const B_ey_y = B.ey.y;
	...
	class b2Mat33 {
		...
		Solve33 (b_x, b_y, b_z, out) {
			const a11 = this.ex.x; const a21 = this.ex.y; const a31 = this.ex.z;
			const a12 = this.ey.x; const a22 = this.ey.y; const a32 = this.ey.z;
			const a13 = this.ez.x; const a23 = this.ez.y; const a33 = this.ez.z;
		...
		GetInverse22 (M) {
			const a = this.ex.x; const b = this.ey.x; const c = this.ex.y; const d = this.ey.y;
			let det = a * d - b * c;
			if (det !== 0) {
				det = 1 / det;
			}
		...
		GetSymInverse33 (M) {
			...
			const a11 = this.ex.x; const a12 = this.ey.x; const a13 = this.ez.x;
			const a22 = this.ey.y; const a23 = this.ez.y;
			const a33 = this.ez.z;
		...
		static MulM33V3 (A, v, out) {
			const v_x = v.x; const v_y = v.y; const v_z = v.z;
	...
	class b2Rot {
		...
		static MulRR (q, r, out) {
			...
			// s = qs * rc + qc * rs
			// c = qc * rc - qs * rs
			const q_c = q.c; const q_s = q.s;
			const r_c = r.c; const r_s = r.s;
    ...
    */
	...
	/*
    * Copyright (c) 2010 Erin Catto http://www.physics.org
    *
    * This software is provided 'as-is', without any express or implied
    * warranty.  In no event will the authors be held liable for any damages
    * arising from the use of this software.
    ...
    */
	...
	class b2PolygonShape extends b2Shape {
		...
		RayCast (output, input, xf, childIndex) {
			...
			let lower = 0; let upper = input.maxFraction;
			let index = -1;
			for (let i = 0; i < this.m_count; ++i) {
				// p = p1 + a * d
				// dot(normal, p - v) = 0
    ...
    */
	...
	class b2Body {
		...
		GetPosition () {
			return this.m_xf ? this.m_xf.p : 0;
		}
		...
		GetAngle () {
			return this.m_sweep && this.m_sweep.a || 0;
		}
    ...
    */
	...
	class b2DistanceJoint extends b2Joint {
		...
		InitVelocityConstraints (data) {
			...
			const qA = this.m_qA.SetAngle(aA); const qB = this.m_qB.SetAngle(aB);
    ...
    */
	...
	class b2FrictionJoint extends b2Joint {
		...
		InitVelocityConstraints (data) {
			...
			const mA = this.m_invMassA; const mB = this.m_invMassB;
			const iA = this.m_invIA; const iB = this.m_invIB;
    ...
    */
	...
	class b2GearJoint extends b2Joint {
		...
		InitVelocityConstraints (data) {
			...
			const qA = this.m_qA.SetAngle(aA); const qB = this.m_qB.SetAngle(aB); const qC = this.m_qC.SetAngle(aC); const qD = this.m_qD.SetAngle(aD);
			this.m_mass = 0;
			if (this.m_typeA === exports.b2JointType.e_revoluteJoint) {
				this.m_JvAC.SetZero();
				this.m_JwA = 1;
		...
		SolvePositionConstraints (data) {
			...
			const linearError = 0;
			let coordinateA, coordinateB;
			const JvAC = this.m_JvAC; const JvBD = this.m_JvBD;
			let JwA, JwB, JwC, JwD;
			let mass = 0;
			if (this.m_typeA === exports.b2JointType.e_revoluteJoint) {
				JvAC.SetZero();
    ...
    */
	...
	class b2ContactFactory {
		...
		Destroy (contact) {
			...
			if (contact.m_swaped) {
				reg = this.m_registers[typeB][typeA];
			} else {
				reg = this.m_registers[typeA][typeB];
			}
			contact.Reset();
			// memory leak end
    ...
    */
	...
	class b2ParticleSystem {
				...
				if (pair.flags & exports.b2ParticleFlag.b2_barrierParticle) {
					...
					while ((c = enumerator.GetNext()) >= 0) {
						...
						if (aGroup !== cGroup && bGroup !== cGroup) {
							...
							let s, t;
							/// b2Vec2 qba, qca;
							const qba = s_qba; const qca = s_qca;
							if (e2 === 0) {
								if (e1 === 0) {
									continue;
								}
		...
		SolveRigidDamping () {
			...
			const invMassA = [0.0]; const invInertiaA = [0.0]; const tangentDistanceA = [0.0]; // TODO: static
			const invMassB = [0.0]; const invInertiaB = [0.0]; const tangentDistanceB = [0.0]; // TODO: static
			...
			ReportFixtureAndParticle (fixture, childIndex, a) {
				...
				if (d < this.m_system.m_particleDiameter && this.ShouldCollideFixtureParticle(fixture, this.m_system, a)) {
					...
					const invAm = this.m_system.m_flagsBuffer.data[a] &
                        exports.b2ParticleFlag.b2_wallParticle
						? 0
