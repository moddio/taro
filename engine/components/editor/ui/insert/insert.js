var UiTextureEditor = IgeEventingClass.extend({
	...
	downloadImage: function () {
		...
		var imageDataElem = form.find('#formImageData');
		var newCanvas;
	...
	resizeBackBuffer: function (newWidth, newHeight) {
		...
		var nbbCtx = newBackBuffer.getContext('2d');
	...
	addImage: function (cell, img) {
		...
		var self = this;
		var x = cell.x * self._cellWidth;
		var y = cell.y * self._cellHeight;
	...
	occupyCells: function (x, y, w, h) {
		var self = this;
		var cellX = Math.floor(x / self._cellWidth);
		var cellY = Math.floor(y / self._cellHeight);
		var cellW = Math.floor(w / self._cellWidth);
		var cellH = Math.floor(h / self._cellHeight);
		var j; var k;
	...
	clearCell: function (cell) {
		var self = this;
		var j; var k; var maxX = 0; var maxY = 0;
