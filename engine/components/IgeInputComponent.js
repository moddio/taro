var IgeInputComponent = IgeEventingClass.extend({
	...
	_rationalise: function (event, touch, debug) {
		...
		event.igeX = (event.igePageX /* - ige._canvasPosition().left */); // left always 0
		event.igeY = (event.igePageY /* - ige._canvasPosition().top */); // top always 0
	...
	_mouseMove: function (event) {
		...
		ige.client.mouseMove = event;

		if (!self.emit('preMouseMove', [event, event.igeX, event.igeY, event.button + 1])) {
			this.queueEvent(this, function () {
	...
	_updateMouseData: function (event) {
		...
		if (!isNaN(event.igeX) || !isNaN(event.igeY)) {
			this.lastMousePosition.x = event.igeX;
			this.lastMousePosition.y = event.igeY;
			resolvedCoordinate.x = event.igeX;
			resolvedCoordinate.y = event.igeY;
		} else {
		...
		if (ige.client.resolutionQuality == 'low') {
			...
			var mx = (resolvedCoordinate.x - ige._bounds2d.x2) - ige._translate.x;
			var my = (resolvedCoordinate.y - ige._bounds2d.y2) - ige._translate.y;
		}

		ige._mouseAbsoluteTranslation = [event.clientX, event.clientY];
		ige._mousePos.x = mx;
		ige._mousePos.y = my;
		// while (arrCount--) {
