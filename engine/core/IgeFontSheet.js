...
var IgeFontSheet = IgeTexture.extend({
		...
		this.on('loaded', function () {
			if (this.image) {
				...
				try {
					...
					this.log(`Could not load data header for font sheet: ${this.image.src} = ${e.toString()}`, 'error');
				}
			}
		});
	},
	...
	_decode: function (canvas, x, y, maxX) {
		'use strict';
		var ctx = canvas.getContext('2d');
		var imageData = ctx.getImageData(x, y, maxX, canvas.height).data;
		var run = true;
		var quadCode;
		var i = 0;
		var jsonString = '';
	...
	measureTextWidth: function (text) {
		if (this._loaded) {
			var characterIndex;
			var charCodeMap = this._charCodeMap;
			var measuredWidthMap = this._measuredWidthMap;
			var charIndex;
			var lineArr = [];
			var lineIndex;
			var measuredWidth;
	...
	render: function (ctx, entity) {
		if (entity._renderText && this._loaded) {
			...
			var characterIndex;
			var charCodeMap = this._charCodeMap;
			var charPosMap = this._charPosMap;
			var measuredWidthMap = this._measuredWidthMap;
			var pixelWidthMap = this._pixelWidthMap;
			var renderX = 0;
			var renderY = 0;
			var renderStartX = 0;
			var renderStartY = 0;
