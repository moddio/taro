...
var IgeCellSheet = IgeTexture.extend({
	...
	init: function (url, horizontalCells, verticalCells, shouldScale, spriteCellSheet, entity) {
		var self = this;
		self.shouldScale = shouldScale;
		self.spriteCellSheet = spriteCellSheet;
	...
	_applyCells: function () {
		...
		if (this.image) {
			...
			if (this._cellRows && this._cellColumns) {
				...
				if (this.shouldScale) {
					imgWidth = this._sizeX * ige.scaleMapDetails.scaleFactor.x;
					imgHeight = this._sizeY * ige.scaleMapDetails.scaleFactor.y;
				}
				rows = this._cellRows;
