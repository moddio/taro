...
var IgeUiEntity = IgeEntity.extend([
	...
	_renderBackground: function (ctx) {
		...
		var left; var top; var width; var height;
	...
	_renderBorder: function (ctx) {
		var rad;
		var geom = this._bounds2d;
		var left = (-(geom.x2) | 0) + 0.5;
		var top = (-(geom.y2) | 0) + 0.5;
		var width = geom.x - 1;
		var height = geom.y - 1;
	...
	tick: function (ctx, dontTransform) {
		if (ige.isClient && !ige.client.uiEntityRenderEnabled) return;
		if (!this._hidden && this._inView && (!this._parent || (this._parent._inView)) && !this._streamJustCreated) {
			...
			if (this._overflow === 'hidden') {
				...
				// of this one
				var geom = this._bounds2d;
				var left = -(geom.x / 2) + this._paddingLeft | 0;
				var top = -(geom.y / 2) + (this._paddingTop) | 0;
				var width = geom.x + this._paddingRight;
				var height = geom.y + this._paddingBottom;
