var ClientNetworkEvents = {
	...
	_onUpdateAllEntities: function (data) {
		...
		for (entityId in data) {
			...
			else { // if user's current browser tab isn't this game.
	...
	_onDevLogs: function (data) {
		ige.variable && ige.variable.updateDevConsole(data)
	},
	...
	_onSound: function (data) {
		...
		else if (data.cmd === 'stopMusicForPlayer') {
			ige.sound.stopMusic();
		}
		else // sound
		{
	...
	_onParticle: function (data) {
		if (data.eid && data.pid) {
			...
			if (
				entity &&
				entity.particleEmitters[data.pid] &&
				entity._translate.x > ige.client.vp1.camera._translate.x - 1000 &&
				entity._translate.x < ige.client.vp1.camera._translate.x + 1000 &&
				entity._translate.y > ige.client.vp1.camera._translate.y - 1000 &&
				entity._translate.y < ige.client.vp1.camera._translate.y + 1000
			) {
			...
			if (
				...
				new IgeParticleEmitter() // Set the particle entity to generate for each particle
					...
					.velocityVector(
						new IgePoint3d(parseFloat(particle.velocityVector.baseVector.x), parseFloat(particle.velocityVector.baseVector.y), 0),
						new IgePoint3d(parseFloat(particle.velocityVector.minVector.x), parseFloat(particle.velocityVector.minVector.y), 0),
						new IgePoint3d(parseFloat(particle.velocityVector.maxVector.x), parseFloat(particle.velocityVector.maxVector.y), 0)
					)
